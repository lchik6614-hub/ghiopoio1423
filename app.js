...
const state = { selected: new Set(), previewAnimation: null };
...
function setProfile(user, balance = null, starsBalance = null) {
  ...
  if (balance !== null) $("#top-balance").textContent = Number(starsBalance ?? balance ?? 0).toFixed(0);
  ...
  if (photoUrl) {
    $("#profile-avatar").innerHTML = `<img src="${photoUrl}" alt="Аватар Telegram" />`;
  } else {
...
function toggleTemplate(id) {
  ...
  showTemplateAnimation(id);
  renderTemplates();
  updatePurchase();
}

function showTemplateAnimation(id) {
  const preview = $("#selected-preview");
  preview.classList.remove("hidden");
  $("#selected-preview-label").textContent = `Анимация шаблона ${id}`;
  if (state.previewAnimation) state.previewAnimation.destroy();
  $("#lottie-inline").innerHTML = "";
  if (window.lottie) {
    state.previewAnimation = window.lottie.loadAnimation({
      container: $("#lottie-inline"),
      renderer: "svg",
      loop: true,
      autoplay: true,
...
setupTelegram();
setProfile(telegram?.initDataUnsafe?.user);
renderTemplates();
initEvents();
updatePurchase();
syncProfile();
