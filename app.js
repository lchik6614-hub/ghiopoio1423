...
function sendToBot(payload) {
  if (payload.action === "topup") {
    const botUsername = query.get("bot");
    const method = payload.method === "stars" ? "stars" : "crypto";
    if (botUsername && telegram?.openTelegramLink) {
      telegram.openTelegramLink(`https://t.me/${encodeURIComponent(botUsername)}?start=art_topup_${method}`);
      return;
    }
    if (!botUsername) {
      showToast("Открой Mini App заново через новое сообщение /start");
      return;
    }
  }
...
function orderStartPayload(text, method) {
  ...
  for (const id of state.selected) {
    ...
    if (index >= 0 && index < 50) bits[Math.floor(index / 8)] |= 1 << (index % 8);
  }
  let binary = "";
...
function choosePayment(method) {
  ...
  const botUsername = query.get("bot");
  if (botUsername && telegram?.openTelegramLink) {
    const payload = orderStartPayload(text, method);
    if (payload.length <= 64) {
      telegram.openTelegramLink(`https://t.me/${encodeURIComponent(botUsername)}?start=${payload}`);
      $("#payment-modal").classList.add("hidden");
      return;
    }
    showToast("Сократи текст до 16 латинских символов для оплаты");
    return;
  }
  if (!botUsername) {
    showToast("Открой Mini App заново через новое сообщение /start");
