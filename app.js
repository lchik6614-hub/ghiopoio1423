  ...
  try {
    if (API_BASE_URL && telegram?.initData) {
      try {
        ...
        // If the public API is temporarily unavailable, use Telegram's
        // native sendData channel below instead of trapping the user in the
        // Mini App.
    ...
    const botUsername = String(query.get("bot") || "").replace(/^@/, "").trim();
    if (botUsername && telegram?.openTelegramLink && payload.action === "create_order") {
      const startPayload = orderStartPayload(payload.text, payload.paymentMethod);
      if (startPayload.length <= 64) {
        telegram.openTelegramLink(`https://t.me/${botUsername}?start=${encodeURIComponent(startPayload)}`);
        return true;
      }
    }
